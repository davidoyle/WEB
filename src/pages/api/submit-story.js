...
const missingEmailEnvs = [
  ['SENDGRID_API_KEY', sendgridApiKey],
  ['STORIES_INBOX', storiesInbox],
...
export default async function handler(req, res) {
  ...
  if (!supabase || missingSupabaseEnvs.length > 0) {
    return res.status(500).json({ ok: false, error: 'Database configuration is missing', missing: missingSupabaseEnvs });
  }

  if (missingEmailEnvs.length > 0) {
    return res.status(500).json({ ok: false, error: 'Email configuration is missing', missing: missingEmailEnvs });
  }
