...
const deriveTagFamilies = (categoryList) => {
  const bodyParts = new Set();
  const issueTags = new Set();

  categoryList.forEach((cat) => {
    cat.cases?.forEach((c) => {
      if (c.bodyPart) bodyParts.add(c.bodyPart);
      getCaseFamilyBuckets(c.issueTags).forEach((family) => issueTags.add(family));
    });
  });

  return {
    allBodyParts: Array.from(bodyParts).sort(),
    allIssueTags: Array.from(issueTags).sort(),
  };
};
...
const WCATToolkit = () => {
  ...
  const { allBodyParts, allIssueTags } = useMemo(() => deriveTagFamilies(categories), []);
