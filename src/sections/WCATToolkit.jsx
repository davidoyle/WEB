...
const deriveTagFamilies = (issueTags = []) => {
...
const WCATToolkit = () => {
    ...
    return categories
      .map((category) => {
        const filteredCases = category.cases?.filter((caseItem) => {
          ...
          const matchesTag =
            !normalizedTagQuery ||
            caseFamilies.some((family) => family.toLowerCase().includes(normalizedTagQuery));
  ...
  }, [query, selectedBodyPart, tagQuery]);
