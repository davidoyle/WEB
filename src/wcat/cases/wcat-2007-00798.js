const caseData = {
  caseNumber: '2007-00798',
  id: 'wcat-2007-00798',
  citation: 'WCAT-2007-00798',
  year: 2007,
  bodyPart: 'general',
  issueTags: ['timelines', 'decision-status', 'communication'],
  phrasesToSteal: [
    'Implementation letters can still be appealable if they resolve entitlement.',
    'Analyze whether the letter changes rights rather than assuming it is informational.',
  ],
  decisionLink: null,
  title: 'Informational letter vs adjudicative decision',
  description:
    'Clarifies when a letter implementing a WCAT decision is merely informational versus a new appealable decision.',
  strategyMoves: [
    'Analyze whether the letter changes rights or simply reports implementation',
    'If it resolves entitlement, argue it’s a decision subject to review',
  ],
  portableStrategy:
    'Push WCAT to label communications as decisions when they resolve issues, even if framed as FYI.',
  category: 'What counts as a decision? Timelines & Board do-overs',
  shortLabel: 'Informational letter vs adjudicative decision',
  fullLabel: 'Informational letter vs adjudicative decision',
  keyPoints: [
    'Implementation letters can still be appealable if they resolve entitlement.',
    'Analyze whether the letter changes rights rather than assuming it is informational.',
  ],
  facts: [
    'Clarifies when a letter implementing a WCAT decision is merely informational versus a new appealable decision.',
  ],
  howToUse: [
    'Analyze whether the letter changes rights or simply reports implementation',
    'If it resolves entitlement, argue it’s a decision subject to review',
  ],
};

export default caseData;
