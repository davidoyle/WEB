    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-[var(--bg-secondary)] p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-[var(--text-muted)]">Feedback</p>
            <h2 className="text-xl font-bold text-[var(--text-primary)]">Was this page helpful?</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="block text-sm font-semibold text-[var(--text-secondary)]" htmlFor="feedback-comment">
            Optional note
            <textarea
              id="feedback-comment"
              value={feedback.comment}
              onChange={e => setFeedback(prev => ({ ...prev, comment: e.target.value }))}
              className="mt-2 w-full rounded-lg border border-gray-300 p-3 text-[var(--text-primary)] focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-500"
              rows={3}
              placeholder="Anything missing or confusing?"
            />
          </label>
          <div className="flex items-center justify-between">
            <span className="text-sm text-[var(--text-muted)]">Stored securely in Supabase.</span>
            <button
              type="submit"
              disabled={!feedback.helpful || status.saving}
              className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[var(--accent-hover)] focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-gray-300"
            >
              {status.saving ? 'Saving…' : 'Submit'}
            </button>
          </div>
          {status.message ? <p className="text-sm text-[var(--text-secondary)]">{status.message}</p> : null}
        </form>
      </div>
    </div>
  );
