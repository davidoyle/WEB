const caseData = {
  caseNumber: '2014-02791',
  id: 'wcat-2014-02791',
  citation: 'WCAT-2014-02791',
  year: 2014,
  bodyPart: 'psych',
  issueTags: ['bullying', 'mental-health', 'significant-stressor'],
  phrasesToSteal: [
    'Bullying and harassment requires abusive or threatening conduct, not just rudeness.',
    'The worker’s psychological injury tracked the targeted acts, not everyday friction.',
  ],
  decisionLink: null,
  title: "Bullying & harassment vs 'just rudeness'",
  description:
    'Explains bullying/harassment must include abusive or threatening behaviour; mere rudeness or thoughtlessness doesn’t qualify.',
  strategyMoves: [
    'Define the threshold using OHS and Board guidance',
    'Sort facts into real bullying/harassment events versus mere rudeness',
    'Link the worker’s condition to the serious events',
  ],
  portableStrategy:
    'Curate evidence by highlighting repeated, targeted acts (threats, put-downs, undermining) and downplaying general rudeness.',
  category: 'Mental disorder, bullying/harassment & late applications',
  shortLabel: "Bullying & harassment vs 'just rudeness'",
  fullLabel: "Bullying & harassment vs 'just rudeness'",
  keyPoints: [
    'Bullying and harassment requires abusive or threatening conduct, not just rudeness.',
    'The worker’s psychological injury tracked the targeted acts, not everyday friction.',
  ],
  facts: [
    'Explains bullying/harassment must include abusive or threatening behaviour; mere rudeness or thoughtlessness doesn’t qualify.',
  ],
  howToUse: [
    'Define the threshold using OHS and Board guidance',
    'Sort facts into real bullying/harassment events versus mere rudeness',
    'Link the worker’s condition to the serious events',
  ],
};

export default caseData;
