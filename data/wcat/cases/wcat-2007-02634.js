const caseData = {
  caseNumber: '2007-02634',
  id: 'wcat-2007-02634',
  citation: 'WCAT-2007-02634',
  year: 2007,
  bodyPart: 'general',
  issueTags: ['parking-lot', 'course-of-employment', 'factors'],
  phrasesToSteal: [
    'Parking-lot coverage depends on ownership/control, purpose of activity, and commuting status.',
    'Use a factor checklist to show the injury remained in the course of employment.',
  ],
  decisionLink: null,
  title: 'Factors for parking-lot injuries',
  description:
    'Sets out factors WCAT considers for parking-lot injuries and summarizes multiple earlier decisions.',
  strategyMoves: [
    'Systematize factors: lot ownership/control, whether commuting is integral, paid time/status, purpose of activity',
    'Build a multi-point argument that the injury was in the course of employment',
  ],
  portableStrategy:
    'Turn the factors into a checklist to show the parking-lot injury stays within course of employment.',
  category: 'Parking lots, travelling workers & ‘off-route’ situations',
  shortLabel: 'Factors for parking-lot injuries',
  fullLabel: 'Factors for parking-lot injuries',
  keyPoints: [
    'Parking-lot coverage depends on ownership/control, purpose of activity, and commuting status.',
    'Use a factor checklist to show the injury remained in the course of employment.',
  ],
  facts: [
    'Sets out factors WCAT considers for parking-lot injuries and summarizes multiple earlier decisions.',
  ],
  howToUse: [
    'Systematize factors: lot ownership/control, whether commuting is integral, paid time/status, purpose of activity',
    'Build a multi-point argument that the injury was in the course of employment',
  ],
};

export default caseData;
