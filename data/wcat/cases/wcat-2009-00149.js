const caseData = {
  caseNumber: '2009-00149',
  id: 'wcat-2009-00149',
  citation: 'WCAT-2009-00149',
  year: 2009,
  bodyPart: 'general',
  issueTags: ['communication', 'timelines', 'appeal-rights'],
  phrasesToSteal: [
    'Simply disclosing file materials is not proper decision communication for limitation purposes.',
    'Timelines do not run until an actual decision is communicated.',
  ],
  decisionLink: null,
  title: 'Disclosure of file is NOT proper decision communication',
  description:
    'Holds that simply disclosing a file is not an appropriate method of communicating a decision for timeline purposes.',
  strategyMoves: [
    'Differentiate disclosure from decision communication',
    'Argue timelines don’t run until a decision is properly communicated',
  ],
  portableStrategy:
    'If you only received file materials, argue the decision wasn’t communicated and timelines haven’t started.',
  category: 'What counts as a decision? Timelines & Board do-overs',
  shortLabel: 'Disclosure of file is NOT proper decision communication',
  fullLabel: 'Disclosure of file is NOT proper decision communication',
  keyPoints: [
    'Simply disclosing file materials is not proper decision communication for limitation purposes.',
    'Timelines do not run until an actual decision is communicated.',
  ],
  facts: [
    'Holds that simply disclosing a file is not an appropriate method of communicating a decision for timeline purposes.',
  ],
  howToUse: [
    'Differentiate disclosure from decision communication',
    'Argue timelines don’t run until a decision is properly communicated',
  ],
};

export default caseData;
